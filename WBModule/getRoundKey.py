...
def write_trace_file(traces, filename='tracefile.txt'):
    """
    Write hex traces to a file under traces/ directory.
    :param traces: Iterable of 16-byte sequences, hex strings, or lists/tuples of ints
    ...
    with open(path, 'wb') as f:
        for entry in traces:
            # entry can be bytes/bytearray, list/tuple of ints, or hex string
            if isinstance(entry, (bytes, bytearray)):
                hex_line = ''.join(f"%02x" % b for b in entry)
            elif isinstance(entry, (list, tuple)):
                hex_line = ''.join(f"%02x" % b for b in entry)
            else:
                # assume hex string without newline
                hex_line = str(entry)
            f.write((hex_line + '\n').encode('utf8'))
    return path


def crack_trace_file(trace_path):
    """
    Call phoenixAES.crack_file on the given trace file.
    :param trace_path: Path to the trace file
    :return: Crack result (e.g., last round key)
    """
    return crack_file(trace_path)


def crack_from_traces(traces, filename='tracefile.txt'):
    """
    Convenience method: write traces then crack.
    :param traces: Iterable of traces
    :param filename: Trace file name
    :return: Crack result
    """
    trace_path = write_trace_file(traces, filename)
    return crack_trace_file(trace_path)
